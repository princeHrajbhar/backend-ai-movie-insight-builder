...
exports.analyzeAudienceSentiment = analyzeAudienceSentiment;
...
async function analyzeAudienceSentiment(reviews) {
    // Create AI client safely
    const ai = getAI();
    if (!reviews.length) {
        return fallback(0);
    }
    const averageUserRating = reviews.reduce((acc, r) => acc + (r.rating || 0), 0) /
        reviews.length;
